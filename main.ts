...
class OptionsModal extends SuggestModal<MigrationTarget> {
	...
	getSuggestions(
		query: string
	): MigrationTarget[] | Promise<MigrationTarget[]> {
		const { folder } =
			// @ts-ignore
			window.app.plugins.getPlugin("periodic-notes")?.settings?.daily ||
			{};
		return new Promise((resolve) => {
			...
			query = query.toLowerCase();
			const parsed = chrono.parseDate(dateQuery, new Date(), {
				// @ts-ignore
				timezone: this.app.vault.getConfig("locale").timezone,
			});
			const date = parsed ? parsed.toISOString().split("T")[0] : null;

			const files: MigrationTarget[] = this.app.vault
				.getMarkdownFiles()
	...
	async onChooseSuggestion(
		...
		if (selections.length === 0) {
			...
			selections.forEach((selection) => {
				if (selection.head.line < selection.anchor.line) {
					for (
						let i = selection.head.line;
						i <= selection.anchor.line;
						i++
					) {
						lines.push(i);
					}
					...
					for (
						let i = selection.anchor.line;
						i <= selection.head.line;
						i++
					) {
						lines.push(i);
					}
		...
		lines.forEach(async (lineNumber) => {
			...
			const newLine =
				line.replace(`- [${taskMatch[2]}] `, `- [${this.type}] `) +
				" " +
				link;
		...
		if (frontmatterIndex !== -1) {
			const frontmatterEnd = content.indexOf(
				"---",
				frontmatterIndex + frontmatter.length
			);
			if (frontmatterEnd !== -1) {
				const newContent =
					content.substring(0, frontmatterEnd + frontmatter.length) +
					"\n" +
					taskContent.join("\n") +
					"\n" +
